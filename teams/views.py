...
class AllTeamsView(APIView):
    ...
    def get(self, request):
        ...
        in_person_teams = InPersonTeam.objects.filter(
            models.Q(leader=user) | models.Q(members__user=user), status="active"
        ).distinct()
        ...
        in_person_data = InPersonTeamSerializer(
            in_person_teams, many=True, context={"request": request}
        ).data
        online_data = OnlineTeamSerializer(
            online_teams, many=True, context={"request": request}
        ).data
...
class InPersonTeamJoinView(APIView):
    ...
    def post(self, request):
        ...
        with transaction.atomic():
            # Create team member
            team_member = TeamMember.objects.create(user=request.user, in_person_team=team)
            team_serializer = InPersonTeamSerializer(team, context={"request": request})

            return Response(
                {"message": "Successfully joined the in-person team", "team": team_serializer.data},
...
class OnlineTeamJoinView(APIView):
    ...
    def post(self, request):
        ...
        with transaction.atomic():
            # Create team member
            team_member = TeamMember.objects.create(user=request.user, online_team=team)
            team_serializer = OnlineTeamSerializer(team, context={"request": request})

            return Response(
                {"message": "Successfully joined the online team", "team": team_serializer.data},
                status=status.HTTP_201_CREATED,
            )
...
class InPersonTeamCreateView(APIView):
    ...
    def post(self, request):
        ...
        if TeamMember.objects.filter(user=request.user, in_person_team__status="active").exists():
            return Response(
        ...
        with transaction.atomic():
            ...
            return Response(
                {"message": "In-person team created successfully", "team": team_serializer.data},
                status=status.HTTP_201_CREATED,
            )
...
class OnlineTeamCreateView(APIView):
    ...
    def post(self, request):
        ...
        if TeamMember.objects.filter(user=request.user, online_team__status="active").exists():
            return Response(
        ...
        with transaction.atomic():
            ...
            return Response(
                {"message": "Online team created successfully", "team": team_serializer.data},
                status=status.HTTP_201_CREATED,
            )
...
class InPersonTeamLeaveView(APIView):
    ...
    def post(self, request, team_id):
        try:
            ...
            return Response({"error": "In-person team not found"}, status=status.HTTP_404_NOT_FOUND)

        if team.leader == request.user:
            team.delete()
            return Response({"message": "Successfully deleted team"}, status=status.HTTP_200_OK)

        try:
            team_member = TeamMember.objects.get(in_person_team=team, user=request.user)
            team_member.delete()
            return Response(
                {"message": "Successfully left the in-person team"}, status=status.HTTP_200_OK
            )
...
class OnlineTeamLeaveView(APIView):
    ...
    def post(self, request, team_id):
        try:
            ...
            return Response({"error": "Online team not found"}, status=status.HTTP_404_NOT_FOUND)
        ...
        try:
            ...
            team_member.delete()
            return Response(
                {"message": "Successfully left the online team"}, status=status.HTTP_200_OK
            )
        except TeamMember.DoesNotExist:
            return Response(
