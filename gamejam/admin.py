...
from .models import OnlineCompetition, OnlineTeam, OnlineMember, OnlineSubmission, MIN_MEMBERS, MAX_MEMBERS
...
class OnlineTeamAdmin(admin.ModelAdmin):
    list_display = ('name', 'leader_info', 'status_display', 'member_count_display', 'invite_code', 'created_at')
...
class OnlineSubmissionAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Submission', {
            'fields': ('title', 'description', 'game_url', 'file')
        }),
        ('Judging', {
    ...
    def game_url_link(self, obj):
        if obj.game_url:
            return format_html(
                '<a href="{}" target="_blank" style="color:#3b82f6;">🔗 View Game</a>',
                obj.game_url
            )
        return '-'
